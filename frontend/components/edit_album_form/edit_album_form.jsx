...
class EditAlbumForm extends React.Component {
  ...
  handleUpdate() {
    ...
    if (album.albumArt && album.title !== "" && tracks.every(hasTitle)) {
      ...
      updateAlbum(albumFormData).then(
        this.setState({ partiallyUpdated: true })
      );
    } else if (album.title === "") {
  ...
  discardTrack(i) {
    ...
    let newTracksCopy = newTracks, albumCopy = album, idx = i - tracks.length;
    newTracksCopy.splice(idx, 1);
    this.setState({ activePanel: 0, newTracks: newTracksCopy, album: albumCopy });
  }
